...
export function LevelsSection() {
  ...
  return (
    <section className="bg-art-cream py-16 sm:py-20 lg:py-28">
            ...
            {LEVELS.map((level, i) => {
              ...
              return (
                ...
                >
                  ...
                  {i < LEVELS.length - 1 && (
                    <div className="absolute right-0 top-1/2 hidden h-px w-4 -translate-y-1/2 translate-x-full bg-art-charcoal/15 lg:block" />
                  )}

                  <div
                    className={cn(
                      'relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm transition-all duration-300',
                      'hover:shadow-md hover:-translate-y-1',
                      'sm:p-7'
                    )}
                  >
                    {/* Level number */}
                    <span className="absolute right-4 top-4 font-[var(--font-cormorant)] text-4xl font-bold text-art-charcoal/5 sm:text-5xl">
                      {level.num}
                    </span>
                    ...
                    {/* Content */}
                    <h3 className="mt-4 text-lg font-semibold text-art-charcoal">
                      {t(`levels.${level.key}.name`)}
                    </h3>
                    <p className="mt-2 text-sm leading-relaxed text-art-charcoal/60">
